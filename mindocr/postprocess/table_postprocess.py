...
class TableMasterLabelDecode(object):
    ...
    def __call__(self, preds, labels=None, *args, **kwargs):
        ...
        shape_list = labels[-1]
        # print('1!!!!type(structure_probs)', type(structure_probs))
        # print('2!!!!type(bbox_preds)', type(bbox_preds))
        # print('3!!!!type(shape_list)', type(shape_list))
        result = self.decode(structure_probs, bbox_preds, shape_list)
        if len(labels) == 1:  # only contains shape
            return result
        # print('4!!!!type(labels)', type(labels))
        label_decode_result = self.decode_label(labels)
        return result, label_decode_result
    ...
    def decode_label(self, batch):
        ...
        structure_idx = batch[1]
        gt_bbox_list = batch[2]
        # print('type(gt_bbox_list)', type(gt_bbox_list))
        shape_list = batch[-1]
        ignored_tokens = self.get_ignored_tokens()
        end_idx = self.dict[self.end_str]
